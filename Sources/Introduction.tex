...
letting the programmers to realize and correct such errors in advance.

In early stages, programming languages like Java (before 1.5)
are built on a simple type system,
where only features like
primitive types, explicitly-typed functions and non-generic classes are supported.
...
\end{verbatim}
Similarly, the swap function for float array can be defined as
\begin{verbatim}
...
\end{verbatim}
which mostly shares the same body with the above function,
...
The \verb|swap2_generic| function utilize the feature of type system, generics,
to improve modularity.
The following program invokes the generic function (suppose we defined
the \verb|swap2_generic| function in a \verb|Utils| class)
\begin{verbatim}
...
thus save them from writing redundant code.
...
Or even more commonly seen in practise,
an accidental misspelled variable name might lead to a runtime error until
the line of code is actually executed.
Type systems are designed to prevent such problem from happening,
therefore statically-typed languages
ensures type-safety, or ``well-typed programs cannot go wrong''.
Type inference algorithms that come along with modern type systems
help eliminate trivial parts and improve conciseness of programs.
...
Functional programming originates from the lambda calculus~\citep{}.
The simply typed lambda calculus~\citep{TAPL} extends the lambda calculus with
...
there exist programs that type checks under one of them and fails under the other.
...
The type system of many programminga languages like Java adopt the idea from HM,
...
where any unannotated program can always be inferred to
...
One of the key features that improves type inference algorithms with type annotations
...
or proofs involving \emph{logical relations}~\cite{abel2018}.

\paragraph{Motivations for Machanical Formalizations.}
The main argument for mechanical formalizations is a simple one.
Proofs for programming languages tend to be \emph{long}, \emph{tedious} and
...
which invalidate the claimed properties.
...
but instead propagate information across judgements.
...
The Abella theorem prover~\citep{AbellaDesc} is one
that specifically ease formalization on binders.
