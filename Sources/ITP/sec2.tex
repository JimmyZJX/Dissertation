...
with $\alpha,\beta$---whenever a monotype $\tau$ needs to be guessed. Below is a sample
...
{\inferrule*[right=$\mathtt{{<:}{\to}}$]
	{
		...
		{\alpha\vdash 1\le \alpha \dashv \alpha=1}
		\quad
		\inferrule*[right=$\mathtt{{<:}Unit}$]{~}
		{\alpha=1\vdash 1\le 1\vdash \alpha=1}
	}
	{\alpha\vdash \alpha\to\alpha \le 1\to 1\dashv \alpha = 1}
}
{\cdot\vdash \forall a. a\to a \le 1 \to 1 \dashv \cdot}$$

\noindent 
The first step applies Rule $\mathtt{{<:}\forall L}$, which introduces a
fresh existential variable, $\alpha$, and opens the left-hand-side
$\forall$-quantifier with it. Next, Rule $\mathtt{{<:}{\to}}$
splits the judgment in two. For the first branch, Rule
$\mathtt{InstRSolve}$ satisfies $1\le \alpha$ by solving $\alpha$ to
